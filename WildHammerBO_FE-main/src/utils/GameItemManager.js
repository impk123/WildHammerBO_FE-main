...
class GameItemManager {
  ...
  getItemIcon(id) {
    ...
    let iconPath = `/bundle/gui/main/atlas/item/${item.icon}.png`
        
    if (item.type === 3) {
      iconPath = `/bundle/gui/main/atlas/skill/${item.icon}.png`
    } else if (item.type === 5) {
      iconPath = `/bundle/gui/yishou/texture/head_icon/${item.icon}.png`
    }
